...
import cn.royan.haxe.responses.users.JoinRoomResponse;
...
class JoinRoomParse extends SimpleParse<{message:Message,socket:Socket}>{
	public function new()
	{
	...
	override public function parse( info:{message:Message,socket:Socket} ):Void
	{
		...
		var joinRoomRequest:JoinRoomRequest = cast info.message;
		
		trace(joinRoomRequest.id);
		trace(joinRoomRequest.pass);
		trace(joinRoomRequest.roomToLeave);
		trace(joinRoomRequest.asSpect);
	...
	{
		return info.message.getCode() == JoinRoomRequest.CODE;
	}
}