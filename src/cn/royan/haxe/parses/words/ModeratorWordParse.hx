...
import cn.royan.haxe.requests.words.ModeratorWordRequest;
...
class ModeratorWordParse extends SimpleParse<{message:Message,socket:Socket}>{
	public function new()
	{
	...
	override public function parse( info:{message:Message,socket:Socket} ):Void
	{
		...
		var moderatorWordRequest:ModeratorWordRequest = cast info.message;
	...
	{
		return (cast info.message).type == ModeratorWordRequest.TYPE;
	}
}