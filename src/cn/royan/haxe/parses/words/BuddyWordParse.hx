...
import cn.royan.haxe.requests.words.BuddyWordRequest;
...
class BuddyWordParse extends SimpleParse<{message:Message,socket:Socket}>{
	public function new()
	{
	...
	override public function parse( info:{message:Message,socket:Socket} ):Void
	{
		...
		var buddyWordRequest:BuddyWordRequest = cast info.message;
	...
	{
		return (cast info.message).type == BuddyWordRequest.TYPE;
	}
}