...
import cn.royan.haxe.requests.words.AdminWordRequest;
...
class AdminWordParse extends SimpleParse<{message:Message,socket:Socket}>{
	public function new()
	{
	...
	override public function parse( info:{message:Message,socket:Socket} ):Void
	{
		...
		var adminWordRequest:AdminWordRequest = cast info.message;
	...
	{
		return (cast info.message).type == AdminWordRequest.TYPE;
	}
}